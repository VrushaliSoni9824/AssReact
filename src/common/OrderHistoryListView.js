...
const OrderHistoryListView = ({item,apiUrl,mediaUrl,catUrl, refreshOrder}) => {
    const [apiStatus, setApiStatus] = useState(false);
    const navigation = useNavigation();    

    var imagesource = mediaUrl + item.item.image_1;  
    ...
    const pauseOrder = async () => { 
        console.log("play-pause API === ", apiUrl+'play-pause')
        fetch(apiUrl+'play-pause',{
            method : 'POST',
            headers : {
    ...
    if(item.item.delivery_status  == 'On Process' || item.item.delivery_status  == 'Pause' ){
        return (
